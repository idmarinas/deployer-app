...
- **Alternativa más simple y robusta, usada en la mayoría de las tabs de proyecto**: `(view).vue` hace `provide('reloadProject', reload)` (el `reload` del propio loader). Cualquier tab/componente puede inyectarlo (`inject<(() => Promise<unknown>) | undefined>('reloadProject')`) y llamarlo (`await reloadProject?.()`) tras cualquier `invoke` exitoso que modifique datos del proyecto. Esto reemplaza `project.value` por un objeto recién leído de la BD (vía `useProjectQuery().find(id)`), garantizando consistencia sin tener que razonar sobre qué mutación concreta dispara o no reactividad. Coste: una consulta extra a SQLite (irrelevante en una app de escritorio local).
- **Excepción:** si una tab mantiene su propia copia local en un `ref()` normal (no shallow) — patrón usado en `ProjectTabTasks.vue` con `localTasks` — esa copia SÍ es profundamente reactiva (`ref()` sobre un array/objeto llama a `reactive()` por debajo) y las mutaciones anidadas sobre ella funcionan bien para el renderizado *de esa tab*. Aun así, cualquier escritura que solo toque `project.value.project_tasks` sin también actualizar la copia local (`localTasks`) o llamar a `reloadProject()` deja la tab visualmente desincronizada del resto de la app.
